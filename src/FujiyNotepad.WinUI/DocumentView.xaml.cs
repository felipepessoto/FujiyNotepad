...
namespace FujiyNotepad.WinUI
{
    public sealed partial class DocumentView : UserControl
    {
        ...
        /// <summary>Native handle of the hosting window, set by the shell so file pickers can parent to it.</summary>
        public nint WindowHandle { get; set; }

        /// <summary>The tab/window title for this document (the open file name, or a placeholder when empty).</summary>
        public string DocTitle { get; private set; } = "(new tab)";

        /// <summary>Raised when <see cref="DocTitle"/> changes, so the shell can update the tab header / window title.</summary>
        public event Action? TitleChanged;

        /// <summary>Raised when the user chooses File &gt; Exit, so the shell can close the window.</summary>
        public event Action? ExitRequested;

        /// <summary>Loads <paramref name="path"/> into this document once it is ready (initial / command-line file).</summary>
        public void OpenPath(string path)
        {
            DispatcherQueue.TryEnqueue(() => { _ = OpenFile(path); });
        }

        /// <summary>Releases this document's file / indexing / watcher resources (shell calls it on tab or window close).</summary>
        public void Cleanup()
        {
            StopWatchingFile();
        ...
        // Reads the persisted settings, applies a change, and saves — so that several open documents (tabs) and
        // the shell can update different fields without one wholesale save clobbering another's changes.
        private void PersistSettings(Action<AppSettings> apply)
        {
            AppSettings latest = settingsStore.Load();
            apply(latest);
        ...
        private async Task OpenFile(string path, bool addToRecent = true, TextEncoding? forcedEncoding = null, bool preserveView = false)
        {
            ...
            SetDocTitle(Path.GetFileName(path));
            ...
            if (addToRecent)
            {
                PersistSettings(s => s.RecentFiles = RecentFiles.Add(s.RecentFiles, path));
                RebuildRecentMenu();
            }
        ...
        private void FindOption_Toggled(object sender, RoutedEventArgs e)
        {
            ...
            PersistSettings(s =>
            {
                s.FindMatchCase = MatchCaseToggle.IsChecked == true;
                s.FindWholeWord = WholeWordToggle.IsChecked == true;
                s.FindUseRegex = RegexToggle.IsChecked == true;
            });
        ...
        private void TabWidth_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                View.TabSize = width;
                PersistSettings(s => s.TabWidth = width);
            }
        }
        ...
        private void LineNumbers_Click(object sender, RoutedEventArgs e)
        {
            ...
            PersistSettings(s => s.ShowLineNumbers = LineNumbersToggle.IsChecked);
        }
        ...
        private void OnFontChanged()
        {
            ...
            PersistSettings(s =>
            {
                s.FontFamily = View.FontFamilyName;
                s.FontSize = View.FontSizePoints;
            });
        }
        ...
        private void RebuildRecentMenu()
        {
            ...
            clear.Click += (_, _) =>
            {
                PersistSettings(s => s.RecentFiles.Clear());
                RebuildRecentMenu();
            };
